...
class DistributedAutonomousDbServiceClientCompositeOperations(object):
    ...
    def rotate_distributed_autonomous_database_passwords_and_wait_for_state(self, distributed_autonomous_database_id, rotate_distributed_autonomous_database_passwords_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        ...
        :param dict waiter_kwargs:
            ...
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.rotate_distributed_autonomous_database_passwords(distributed_autonomous_database_id, rotate_distributed_autonomous_database_passwords_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        if 'opc-work-request-id' not in operation_result.headers:
    ...
    def scale_distributed_autonomous_database_gsms_and_wait_for_state(self, scale_distributed_autonomous_database_gsms_details, distributed_autonomous_database_id, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.distributed_database.DistributedAutonomousDbServiceClient.scale_distributed_autonomous_database_gsms` and waits for the :py:class:`~oci.distributed_database.models.WorkRequest`
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.distributed_database.DistributedAutonomousDbServiceClient.scale_distributed_autonomous_database_gsms`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.scale_distributed_autonomous_database_gsms(scale_distributed_autonomous_database_gsms_details, distributed_autonomous_database_id, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        if 'opc-work-request-id' not in operation_result.headers:
